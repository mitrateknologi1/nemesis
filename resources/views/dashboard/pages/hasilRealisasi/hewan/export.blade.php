                ...
                <tr style="vertical-align: center;border: 1px solid black;">
                    ...
                    <td style="vertical-align: center;border: 1px solid black;" align="left">
                        @forelse ($item->listIndikator as $item2)
                            @if ($tahun_filter && $tahun_filter != 'Semua')
                                @if ($item2->realisasiHewan->created_at->format('Y') == $tahun_filter)
                                    <p>{{ $no_1 }}.
                                        {{ $item2->realisasiHewan->perencanaanHewan->sub_indikator }}</p>
                                    @php
                                        $no_1++;
                                    @endphp
                                @endif
                            @else
                                <p>{{ $loop->iteration }}. {{ $item2->realisasiHewan->perencanaanHewan->sub_indikator }}
                                </p>
                            @endif
                        @empty
                            <p>-</p>
                    ...
                    <td style="vertical-align: center;border: 1px solid black;" align="left">
                        @forelse ($item->listIndikator as $item2)
                            @if ($tahun_filter && $tahun_filter != 'Semua')
                                @if ($item2->realisasiHewan->created_at->format('Y') == $tahun_filter)
                                    <p>{{ $no_2 }}. {{ $item2->realisasiHewan->perencanaanHewan->opd->nama }}
                                    </p>
                                ...
                                @endif
                            @else
                                <p>{{ $loop->iteration }}. {{ $item2->realisasiHewan->perencanaanHewan->opd->nama }}
                                </p>
                                @if ($item2->realisasiHewan->perencanaanHewan->opdTerkaitHewan)
                                    @foreach ($item2->realisasiHewan->perencanaanHewan->opdTerkaitHewan as $item3)
                                        <p>-{{ $item3->opd->nama }}</p>
