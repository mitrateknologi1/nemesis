...
@section('subTitlePanelHeader')
    {{ $realisasi_intervensi_keong->perencanaanKeong->sub_indikator }}
@endsection
                ...
                <div class="card-body">
                    <ul class="list-group list-group-bordered">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">Sub Indikator:<span
                                class="font-weight-bold">{{ $realisasi_intervensi_keong->perencanaanKeong->sub_indikator }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">Jumlah Titik
                            Lokasi:<span
                                class="font-weight-bold">{{ $realisasi_intervensi_keong->lokasiRealisasiKeong->count() }}
                            </span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">OPD Pembuat:<span
                                class="font-weight-bold">{{ $realisasi_intervensi_keong->perencanaanKeong->opd->nama }}</span>
                        </li>
                        @if ($realisasi_intervensi_keong->perencanaanKeong->opdTerkaitKeong->count() > 0)
                            <li class="list-group-item d-flex justify-content-between align-items-center">OPD Terkait
                                ({{ $realisasi_intervensi_keong->perencanaanKeong->opdTerkaitKeong->count() }}):<span
                                    class="font-weight-bold">
                                    <ul>
                                        @foreach ($realisasi_intervensi_keong->perencanaanKeong->opdTerkaitKeong as $item)
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            {{ $realisasi_intervensi_keong->perencanaanKeong->status == 1 ? 'Nilai Anggaran:' : 'Rencana Anggaran:' }}<span
                                class="font-weight-bold"><span>Rp. </span>
                                <span
                                    class="rupiah">{{ $realisasi_intervensi_keong->perencanaanKeong->nilai_pembiayaan }}</span></span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">Sumber Dana:<span
                                class="font-weight-bold">{{ $realisasi_intervensi_keong->perencanaanKeong->sumberDana->nama }}</span>
                        </li>
    ...
    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <div class="card-head-row">
                        <div class="card-title">Titik Lokasi Realisasi Intervensi Habitat Keong</div>
                                            ...
                                            <tbody>
                                                @foreach ($realisasi_intervensi_keong->lokasiRealisasiKeong as $item)
                                                    <tr>
                                                        ...
                                                        <td>{{ $item->lokasiKeong->desa->nama }}</td>
                                                        <td>{{ $item->lokasiKeong->deskripsi == '' ? '-' : $item->lokasiKeong->deskripsi }}
                                                        </td>
                                                        <td>{{ $item->lokasiKeong->latitude }} /
                                                            {{ $item->lokasiKeong->longitude }}</td>
                                                        <td>
                                                            @forelse ($item->lokasiKeong->pemilikLokasiKeong as $item2)
...
@push('scripts')
    <script>
        ...
        function initializeMap() {
            ...
            $.ajax({
                url: "{{ url('realisasi-intervensi-keong/map/' . $realisasi_intervensi_keong->id) }}",
                type: "GET",
                success: function(response) {
                    if (response.status == 'success') {
                        for (var i = 0; i < response.data.length; i++) {
