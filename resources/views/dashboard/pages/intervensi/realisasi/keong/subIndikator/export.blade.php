                ...
                <tr style="vertical-align: center;border: 1px solid black;">
                    ...
                    <td style="vertical-align: center;border: 1px solid black;" align="right">
                        @php
                            ...
                            foreach ($item->realisasiKeong->where('status', 1) as $row) {
                    ...
                    <td style="vertical-align: center;border: 1px solid black;" align="center">
                        @if ($item->realisasiKeong->where('tw', 1)->where('status', 1)->count() > 0)
                            {{ $item->realisasiKeong->where('tw', 1)->where('status', 1)->max('progress') }}%
                        @else
                            -
                        @endif
                    </td>
                    <td style="vertical-align: center;border: 1px solid black;" align="center">
                        @if ($item->realisasiKeong->where('tw', 2)->where('status', 1)->count() > 0)
                            {{ $item->realisasiKeong->where('tw', 2)->where('status', 1)->max('progress') }}%
                        @else
                            -
                        @endif
                    </td>
                    <td style="vertical-align: center;border: 1px solid black;" align="center">
                        @if ($item->realisasiKeong->where('tw', 3)->where('status', 1)->count() > 0)
                            {{ $item->realisasiKeong->where('tw', 3)->where('status', 1)->max('progress') }}%
                        @else
                            -
                        @endif
                    </td>
                    <td style="vertical-align: center;border: 1px solid black;" align="center">
                        @if ($item->realisasiKeong->where('tw', 4)->where('status', 1)->count() > 0)
                            {{ $item->realisasiKeong->where('tw', 4)->where('status', 1)->max('progress') }}%
                        @else
                            -
                        @endif
                    </td>
                    <td style="vertical-align: center;border: 1px solid black;" align="center">
                        @if ($item->realisasiKeong->where('status', 1)->max('progress') == 100)
                            <p>Selesai Terealisasi</p>
                        @else
                            @if ($item->realisasiKeong->where('status', 0)->count() > 0)
                                <p>Menunggu Konfirmasi : </p>{{ $item->realisasiKeong->where('status', 0)->count() }}
                            @endif
                            @if ($item->realisasiKeong->where('status', 1)->count() > 0)
                                <p>Laporan Disetujui : </p>{{ $item->realisasiKeong->where('status', 1)->count() }}
                            @endif
                            @if ($item->realisasiKeong->where('status', 2)->count() > 0)
                                <p>Laporan Ditolak : </p>{{ $item->realisasiKeong->where('status', 2)->count() }}
                            @endif
                            @if ($item->realisasiKeong->count() == 0)
